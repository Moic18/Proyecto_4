function visitas_hotel() {
    ...
    axios({
        method: 'GET',
        url: 'http://127.0.0.1:3000/Contar_estudiantes'
        ...
        for (let i = 0; i < response.data.length; i++) {
            const uno = response.data[i].carrera; // Nombre obtenido desde la API
            const dos = response.data[i].Total_de_estudiantes; // Asistencia obtenida desde la API
            labels.push(uno);
            data.push(dos);
        }
        let b = 0
